...
with webdriver.Chrome(options=chrome_options) as driver:
    ...
    for cookie in cookies:
        ...
        skills_list = len([skill.text for skill in wait.until(EC.visibility_of_all_elements_located(SKILLS))])
        age = int(wait.until(EC.visibility_of_element_located(AGE)).text.replace('Age: ', ''))
        person.setdefault(cookie['value'], []).extend([skills_list, age])

    result = [k for k, _ in sorted(person.items(), key=lambda x: (x[1]))][0]
